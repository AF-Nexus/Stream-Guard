...
// ── Import: CDN Live TV channel list proxy (avoids browser CORS) ─────────────
router.get("/admin/import/cdnlivetv", requireAuth, requireAdmin, async (_req, res) => {
  try {
    ...
    const data = await r.json() as { total_channels?: number; channels?: { name: string; code: string; url: string; image: string; status: string; viewers: number }[] };
    res.json(data);
  } catch (e) {
    res.status(502).json({ error: "Failed to reach CDN Live TV API" });
  }
});
