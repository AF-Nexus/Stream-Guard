...
// Raw GitHub URL pattern — uses the AF-Nexus repo
...
// Parse M3U text → array of channel objects
function parsePlutoM3U(text: string, region: PlutoRegion) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    results.push({ plutoTvId: tvgId, name, logo, group, url });
  }
  return results;
}

// Core sync — returns { added, updated, removed }
async function runPlutoSync(region: PlutoRegion, githubUser = "AF-Nexus") {
  ...
  const text = await r.text();
  const channels = parsePlutoM3U(text, region);
  if (channels.length === 0) throw new Error("Parsed 0 channels — aborting sync to avoid data loss");

  // Build category lookup: group-title → categoryId (auto-create if missing)
  const catCache = new Map<string, string>();

  async function getOrCreateCategory(group: string) {
    const key = group || "Random";
  ...
  for (const ch of channels) {
    ...
    if (existing[0]) {
      // Update name, logo, url, category
      await db.update(channelsTable).set({
        name: ch.name,
        logoUrl: ch.logo,
        sourceUrl: ch.url,
        categoryId,
        sourceReferer: "https://pluto.tv/",
        plutoTvRegion: region,
  ...
  // Delete channels from this region no longer in playlist
  const regionChannels = await db.select({ id: channelsTable.id, plutoTvId: channelsTable.plutoTvId })
    .from(channelsTable).where(eq(channelsTable.plutoTvRegion, region));
  let removed = 0;
  for (const ch of regionChannels) {
    if (ch.plutoTvId && !seenIds.has(ch.plutoTvId)) {
      await db.delete(channelsTable).where(eq(channelsTable.id, ch.id));
      removed++;
    }
  }
