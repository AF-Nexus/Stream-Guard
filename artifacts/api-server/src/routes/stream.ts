...
async function fetchUpstream(url: string, req: Request, refererOverride?: string | null): Promise<globalThis.Response> {
  ...
  // Priority: stored sourceReferer > ?ref= query param > auto-derive from URL origin
  const referer =
    refererOverride ||
    (req.query["ref"] as string | undefined) ||
    (() => { try { const u = new URL(url); return u.origin + "/"; } catch { return undefined; } })();
...
router.get("/p/:token/index.m3u8", async (req: Request, res: Response) => {
  ...
  try {
    upstream = await fetchUpstream(c.sourceUrl, req, c.sourceReferer);
  } catch (err) {
...
router.get("/p/:token/seg", async (req: Request, res: Response) => {
  ...
  // Look up channel so we can forward the correct Referer for token-protected CDNs
  const [c] = await db.select().from(channelsTable).where(eq(channelsTable.id, payload.cid)).limit(1);
  const refererOverride = c?.sourceReferer ?? null;

  let upstream: globalThis.Response;
  try {
    upstream = await fetchUpstream(absUrl, req, refererOverride);
  } catch (err) {
